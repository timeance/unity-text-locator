...
def detect_file_encoding(file_path) -> str:
    """[ainiee-translate patch] Lightweight encoding sniff (chardet optional),
    replacing ReaderUtil.detect_file_encoding so the vendored reader needs no
    mediapipe / Config dependency."""
    try:
        import chardet
        return chardet.detect(Path(file_path).read_bytes()).get("encoding") or "utf-8"
    except Exception:
        return "utf-8"
...
class BaseSourceReader(ABC):
    ...
    def post_read_source(self, file_data: CacheFile) -> CacheFile:
        """[ainiee-translate patch] MediaPipe language detection removed — we
        never read lang_code and iter_untranslated filters on status only, so
        this drops the mediapipe / rich-progress dependency entirely."""
        return file_data
